...
import lemon.elastic.query4j.esproxy.geo.Metric;
...
public enum DistanceFormatter implements Converter<String, Distance>, Formatter<Distance> {
    ...
    public Distance parse(String text, Locale locale) throws ParseException {
        return !SpringStringUtils.hasText(text) ? null : doConvert(text.trim().toLowerCase(locale));
    }
    ...
    private static Distance doConvert(String source) {
        ...
        try {
            ...
            throw new IllegalArgumentException(String.format(INVALID_DISTANCE, SpringStringUtils.collectionToCommaDelimitedString(SUPPORTED_METRICS.keySet()), source));
        }
    }
