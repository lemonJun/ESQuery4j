...
import lemon.elastic.query4j.esproxy.annotations.AccessType;
import lemon.elastic.query4j.esproxy.annotations.AccessType.Type;
import lemon.elastic.query4j.esproxy.annotations.Id;
import lemon.elastic.query4j.esproxy.annotations.Reference;
import lemon.elastic.query4j.esproxy.annotations.Transient;
import lemon.elastic.query4j.esproxy.annotations.Value;
import lemon.elastic.query4j.esproxy.annotations.Version;
...
/**
 * Special {@link PersistentProperty} that takes annotations at a property into
 * account.
 * 基于注解的属性配置    当解析注解时 生成此对象
 * 
...
public abstract class AnnotationBasedPersistentProperty<P extends PersistentProperty<P>> extends AbstractPersistentProperty<P> {
    ...
    /**
     * Populates the annotation cache by eagerly accessing the annotations
     * directly annotated to the accessors (if available) and the backing field.
     * Annotations override annotations found on field.
     * 
     * @param field
     * @throws MappingException
     *         in case we find an ambiguous mapping on the accessor methods
     */
    ...
    /**
     * Verifies the given annotation candidate detected. Will be rejected if
     * it's a Spring Data annotation and we already found another one with a
     * different configuration setup (i.e. other attribute values).
     * 
     * @param candidate
     *        must not be {@literal null}.
     * @param message
    ...
    /**
     * Inspects a potentially available {@link Value} annotation at the property
     * and returns the {@link String} value of it.
     * 
     * @see lemon.elastic.query4j.esproxy.mapping.model.AbstractPersistentProperty#getSpelExpression()
     */
    @Override
     ...
     * , {@link Value} or {@link Autowired} as transien.
     * 
    ...
    public boolean isTransient() {
        ...
        if (this.isTransient == null) {
            ...
            //ZTODO
            //            this.isTransient = potentiallyTransient || isAnnotationPresent(Value.class) || isAnnotationPresent(Autowired.class);
     ...
     * annotations over ones found at the backing field as the former can be
     * used to reconfigure the metadata in subclasses.
     * 
