...
class DefaultPersistentPropertyPath<T extends PersistentProperty<T>> implements PersistentPropertyPath<T> {
    ...
    public DefaultPersistentPropertyPath<T> append(T property) {
        ...
        Preconditions.checkState(property.getOwner().getType().equals(leafPropertyType), String.format("Cannot append property %s to type %s!", property.getName(), leafPropertyType.getName()));
    ...
    public String toPath(String delimiter, Converter<? super T, String> converter) {
        ...
        @SuppressWarnings("unchecked")
        Converter<? super T, String> converterToUse = (Converter<? super T, String>) (converter == null ? PropertyNameConverter.INSTANCE : converter);
        ...
        for (T property : properties) {
            ...
            if (SpringStringUtils.hasText(convert)) {
                result.add(convert);
            }
        }

        return result.isEmpty() ? null : SpringStringUtils.collectionToDelimitedString(result, delimiterToUse);
    }
