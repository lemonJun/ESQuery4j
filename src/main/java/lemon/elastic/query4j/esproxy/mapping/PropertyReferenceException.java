...
public class PropertyReferenceException extends RuntimeException {
    ...
    public PropertyReferenceException(String propertyName, TypeInformation<?> type, List<PropertyPath> alreadyResolvedPah) {
        ...
        Preconditions.checkState(SpringStringUtils.hasText(propertyName), "Property name must not be null!");
        Preconditions.checkNotNull(type, "Type must not be null!");
        Preconditions.checkNotNull(alreadyResolvedPah, "Already resolved paths must not be null!");
    ...
    public String getMessage() {
        ...
        StringBuilder builder = new StringBuilder(String.format(ERROR_TEMPLATE, propertyName, type.getType().getSimpleName()));

        if (!propertyMatches.isEmpty()) {
            String matches = SpringStringUtils.collectionToDelimitedString(propertyMatches, ",", "'", "'");
